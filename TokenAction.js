// Forked from TokenAction 0.2.5 - https://github.com/git-kor-av/TokenAction by Kevin - https://app.roll20.netusers/399372/kevin
...
var tokenAction = tokenAction || (function() {
    ...
    if (attribute == undefined) {
    		var tmpAttr = createObj("attribute", {
    		name: attr_name,
    		characterid: char_id
    		});
    		tmpAttr.setWithWorker({
                current: newVal.toString(),
            });
    		} else {
    	    attribute.setWithWorker("current", newVal.toString());
    	}
    },	
		...
		if (msg.type === 'api' && msg.content.search(/^!ta\b/) !== -1 && msg.selected) {
			...
			_.each(char, function(a) {
			    if (isNpc(a.id) === "1") {
			        ...
			        // Yes to get the stupid sheet worker script to update you have to set it to something (like something you don't want) then to what you want ¯\_(ツ)_/¯
					...
					setAttribute(a.id, "rtype", "{{normal=1}} {{r2=[[0d20");
